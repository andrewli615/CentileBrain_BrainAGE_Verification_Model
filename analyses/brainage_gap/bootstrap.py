...
def plot_histogram(label, deltas, observed, ci, contains_zero):
    ...
    ax.axvline(0, color="#252525", linestyle=":", linewidth=2, label="Zero Difference")
    ...
    result = "CI Contains 0" if contains_zero else "CI Excludes 0"
    ax.set(title=f"{label}\nTwo-Sample Bootstrap (Signed Mean BAG) | {result}",
           xlabel="Cohort mean BAG - reference mean BAG (years)", ylabel="Density")
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    output = folder / f"{label}_bootstrap_histogram.png"
...
def main():
    ...
    for group, sex, age_group, path in cohorts():
        test = pd.read_csv(path)["Adjusted_BrainAGE"].dropna().to_numpy()
        ref = reference_bag(sex, age_group)
        ...
        plot = plot_histogram(label, deltas, observed, ci, contains_zero)
        summaries.append({"Group": group, "Sex": sex, "Age_Group": age_group,
                          "N_Test": len(test), "N_Ref": len(ref), "Test_Mean_BAG": test.mean(),
                          "Ref_Mean_BAG": ref.mean(), "Observed_Delta": observed,
        ...
        samples.append(pd.DataFrame({"Iteration": np.arange(1, N_REPS + 1), "Group": group,
                                     "Sex": sex, "Age_Group": age_group, "Bootstrap_Delta": deltas}))
